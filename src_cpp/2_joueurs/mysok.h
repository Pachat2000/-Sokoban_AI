...
inline reponse sok_board_t::move(vector<char> list_moves,
 ...
 // S.copy(board);
  for (unsigned int i = 0; i < list_moves.size(); i++) {
    ...
    if (str == 'l' || str == 'o') {
     this->sous_func_move(temp_pos_man1_x, temp_pos_man1_y, 0, -1,
      ...
      this->sous_func_move(temp_pos_man1_x, temp_pos_man1_y, 0, 1, is_man_on_target,
                       1);
      temp_pos_man1_y += 1;
    } else if (str == 'd' || str == 'n') {
      this->sous_func_move(temp_pos_man1_x, temp_pos_man1_y, 1, 0, is_man_on_target,
                       1);
      temp_pos_man1_x += 1;
    } else if (str == 'u' || str == 'm') {
    ...
    if (this->board[temp_pos_man2_x][temp_pos_man2_y] == MAN2_ON_TARGET) {
      is_man_on_target2 = TARGET;
    } else {
      is_man_on_target2 = FREE;
    }
    if (str == 'g' || str == 'z') {
      this->sous_func_move(temp_pos_man2_x, temp_pos_man2_y, 0, -1,
                       is_man_on_target2, 2);
      temp_pos_man2_y -= 1;
    } else if (str == 't' || str == 'w') {
      this->sous_func_move(temp_pos_man2_x, temp_pos_man2_y, 0, 1,
      ...
      this->sous_func_move(temp_pos_man2_x, temp_pos_man2_y, 1, 0,
                       is_man_on_target2, 2);
      temp_pos_man2_x += 1;
    } else if (str == 'h' || str == 'x') {
      this->sous_func_move(temp_pos_man2_x, temp_pos_man2_y, -1, 0,
  ...
  man2_x = temp_pos_man2_x;
  man2_y = temp_pos_man2_y;
  reponse t;
  t.win = this->verife_win(vec_finish);
  t.move =
      this->move_option(impossi_move, list_moves.at(list_moves.size() - 1), joueur);
  return t;
}

...
/*position sok_board_t::research_man_position(){
  for(int i = 0; i < board_nbl; i++) {
    for(int j = 0; j < NBC; j++) {
      if(board[i][j]==END_OF_LINE) break;
      if ((board[i][j]==MAN1_ON_FREE)||(board[i][j]==MAN1_ON_TARGET)) return position(i,j);
    }
  }
}*/
/* sok */
  ...
  for(unsigned int i = 0; i < list_moves.size(); i++){
    ...
    if ( str == "left"){
      if (S.board[temp_pos_man1_x][temp_pos_man1_y] == MAN1_ON_TARGET){
          ...
          if ((S.board[temp_pos_man1_x][temp_pos_man1_y-1] == FREE) ||(S.board[temp_pos_man1_x][temp_pos_man1_y-1] == CRATE_ON_FREE)){
            next_man_position = MAN1_ON_FREE;
          }else{
            next_man_position =  MAN1_ON_TARGET;
          }
          /* Man moves alone*/
            if((S.board[temp_pos_man1_x][temp_pos_man1_y-1] == FREE) || (S.board[temp_pos_man1_x][temp_pos_man1_y-1] == TARGET)){  
        ...
        if (S.board[temp_pos_man1_x][temp_pos_man1_y] == MAN1_ON_TARGET){
          ...
          if ((S.board[temp_pos_man1_x+1][temp_pos_man1_y] == FREE) ||(S.board[temp_pos_man1_x+1][temp_pos_man1_y] == CRATE_ON_FREE)){
            next_man_position = MAN1_ON_FREE;
          }else{
            next_man_position =  MAN1_ON_TARGET;
          }
          /* Man moves alone*/
            if((S.board[temp_pos_man1_x+1][temp_pos_man1_y] == FREE) || (S.board[temp_pos_man1_x+1][temp_pos_man1_y] == TARGET)){  
